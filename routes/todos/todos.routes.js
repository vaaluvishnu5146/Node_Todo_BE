...
async function getTodos(request, response) {
  ...
  try {
    ...
    const todosData = await client
      ...
      .collection("todos")
      .find({
        isDeleted: query["isDeleted"] ? Boolean(query["isDeleted"]) : false,
      })
      .toArray();
    await client.close();
    return response
...
async function getTodosV2(request, response) {
  try {
    ...
    if (results.length < 1) {
      return response.status(200).json({ message: "No Todos found", data: [] });
    } else {
      return response
        .status(200)
...
async function getTodosByUserId(request, response) {
  ...
  try {
    const results = await TodosModel.find({ userId: userId })
...
async function getTodoById(request, response) {
  ...
  if (!todoId) {
    ...
    if (result) {
      ...
      return response.status(200).json({ message: "Todo not found", data: {} });
    }
  }
}
...
async function deleteTodo(request, response) {
  try {
    ...
    if (!todoId) {
      ...
      const acknowledgement = await TodosModel.findByIdAndDelete({
      ...
      if (acknowledgement) {
        ...
        return response.status(200).json({
          message: "Todos is either deleted already or not available",
        });
      }
    }
  } catch (error) {
